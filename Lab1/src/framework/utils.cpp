...
void launchLoop(Application* app)
{
	...
	while (1)
	{
		...
		memcpy(app->prev_keystate, app->current_keystate, SDL_NUM_SCANCODES);
		memcpy((void*)&(app->current_keystate), app->keystate, SDL_NUM_SCANCODES);
		...
		app->app_time = (now - start_time) * 0.001;
